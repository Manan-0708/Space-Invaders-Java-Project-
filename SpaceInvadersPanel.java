...
public class SpaceInvadersPanel extends JPanel implements ActionListener, KeyListener {
    ...
    // New variable for the background image
    BufferedImage bgImg;
    ...
    public SpaceInvadersPanel() {
        ...
        // setBackground(Color.black); // Removed, background image will cover this
        setFocusable(true);
        addKeyListener(this);

        try {
            bgImg = ImageIO.read(new File("bgimg.png")); // Load the background image
        } catch (IOException e) {
            System.err.println("Error loading background image: bgimg.png");
            e.printStackTrace();
            bgImg = null; // Handle the case where the image fails to load
        }
    ...
    protected void paintComponent(Graphics g) {
        ...
        if (bgImg != null) {
            ...
            // Fallback to black background if the image didn't load
            g.setColor(Color.black);
            g.fillRect(0, 0, boardWidth, boardHeight);
        }

        if (ship.alive) g.drawImage(ship.img, ship.x, ship.y, ship.width, ship.height, this);
        for (Block alien : alienArray) if (alien.alive) g.drawImage(alien.img, alien.x, alien.y, alien.width, alien.height, this);

        g.setColor(Color.WHITE);
        for (Block bullet : bulletArray) g.fillRect(bullet.x, bullet.y, bullet.width, bullet.height);

        g.setColor(Color.RED);
        for (Block bullet : alienBulletArray) g.fillRect(bullet.x, bullet.y, bullet.width, bullet.height);

        g.setColor(Color.WHITE);
        g.drawString("Score: " + score, 10, 20);

        if (gameOver) g.drawString("Game Over! Press any key to restart", boardWidth / 2 - 100, boardHeight / 2);
    }
